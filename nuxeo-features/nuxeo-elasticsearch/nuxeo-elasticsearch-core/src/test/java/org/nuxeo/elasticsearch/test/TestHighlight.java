...
public class TestHighlight {
    ...
    public void testHighlight() throws Exception {
        ...
        NxQueryBuilder queryBuilder = new NxQueryBuilder(session)
                                                                 ...
                                                                 .highlight(ess.getHighlightFields());
        queryBuilder.updateRequest(request);
        DocumentModelList ret = ess.query(queryBuilder);

        assertEquals(2, ret.totalSize());

        Map<String, List<String>> highlights = (Map<String, List<String>>) ret.get(0).getContextData("highlight");
        assertEquals(2, highlights.size());
        assertTrue(highlights.containsKey("dc:title.fulltext"));
        assertTrue(highlights.containsKey("ecm:binarytext.fulltext"));
        assertEquals("<em>Search</em> me", highlights.get("dc:title.fulltext").get(0));
        assertEquals("you know for <em>search</em>", highlights.get("ecm:binarytext.fulltext").get(0));

        Map<String, List<String>> highlights2 = (Map<String, List<String>>) ret.get(1).getContextData("highlight");
        assertEquals("test my <em>search</em> with highlight", highlights2.get("ecm:binarytext.fulltext").get(0));
    }
    ...
    public void testMultipleHighlights() throws Exception {
        ...
        assertEquals(3, highlights.get("ecm:binarytext.fulltext").size());
        assertEquals(
                "nulla neque dui egestas sit amet nibh eget maximus <em>vehicula</em> nisi aenean non eros id mauris imperdiet tristique",
                highlights.get("ecm:binarytext.fulltext").get(0));
        assertEquals(
                "est nunc proin ac libero <em>vehicula</em> mollis turpis quis tempus sem donec <em>vehicula</em> ante eget porttitor hendrerit",
                highlights.get("ecm:binarytext.fulltext").get(1));
        assertEquals(
                "eros eu rutrum semper nam auctor enim vel lacinia <em>vehicula</em> leo leo porttitor massa in blandit tortor ante",
                highlights.get("ecm:binarytext.fulltext").get(2));
